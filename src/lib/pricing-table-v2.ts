 ...
 */
export function getAllCategoriesWithItems(): Record<string, Record<string, PricingItem>> {
  const categories: Record<string, Record<string, PricingItem>> = {};
  for (const [key, item] of Object.entries(PRICING_TABLE)) {
...
export const CATEGORY_CONFIG: Record<string, { label: string; order: number }> = {
  demo: { label: "Demolition & Site Prep", order: 1 },
  framing: { label: "Structural & Framing", order: 2 },
  plumbing: { label: "Plumbing", order: 3 },
  electrical: { label: "Electrical", order: 4 },
  tile: { label: "Tile & Flooring", order: 5 },
  cabinetry: { label: "Cabinetry & Vanities", order: 6 },
  countertops: { label: "Countertops", order: 7 },
  fixtures: { label: "Fixtures & Allowances", order: 8 },
  glass: { label: "Glass", order: 9 },
  paint: { label: "Paint & Drywall", order: 10 },
  trim: { label: "Finish Carpentry & Trim", order: 11 },
  doors: { label: "Doors", order: 12 },
  mechanical: { label: "Mechanicals & Appliances", order: 13 },
  admin: { label: "Admin & Miscellaneous", order: 14 },
  allowance: { label: "Material Allowances", order: 15 },
};
